...
- One visual decision tied to the subject or reader job.
...
Default to `bg-white`. Use off-white, tinted, dark, or textured backgrounds only for user preference, brand/source palette, subject motif, print requirement, or hierarchy. Use Tailwind stock colors: dark text, muted text, light line, one accent scale, at most one supporting tint. Every added hue needs a job.
...
Mark assumptions, dates, freshness, scope, confidence, sources, and placeholder data when they affect interpretation. Do not invent metrics, ratings, examples, or benchmarks to fill a layout. Charts/SVGs need units, labels or legend, and a takeaway caption.
...
3. Check keyboard/focus, labels, contrast, touch targets, mobile overflow, long text, print/readability, and semantic HTML.
4. If it still feels generated, fix structure and copy before changing colors, shadows, or fonts.
